...
class TipBubble: UIView {
    ...
    func layout() -> CGRect {
        ...
        let frame = CGRect(x: (UIScreen.main.bounds.width - width) / 2 - 10, y: (UIScreen.main.bounds.height * 3 / 4) - height - 10, width: width + 20, height: height + 20)
        self.backgroundColor = UIColor.colorFromHex(hexString: "#5F3ABB").withAlphaComponent(0.4)
    ...
    func show(dalay: Double) {
        ...
        let start_endFrame = CGRect(x: showFrame.origin.x, y: UIScreen.main.bounds.height, width: showFrame.width, height: showFrame.height)
